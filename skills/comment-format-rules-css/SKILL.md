...
**例外：技術術語的雙語標註仍應保留**（如 `flex 布局 (flex layout)`、`偽元素 (pseudo-element)`）。
...
/* ✅ 雙語過長時，選最清晰的語言 */
/*
 * 使用負 margin 抵消父容器的 padding，使背景色延伸至容器邊緣，
 * 避免因 padding 造成視覺上的留白斷層
 */
margin: 0 -20px;

/* ✅ 技術術語仍須保留雙語 */
/* 使用偽元素 (pseudo-element) 建立視覺分隔線，避免修改 DOM 結構 */
&::before { content: ''; }
```
